...
public class Mouse extends AbstractMouse {
     ...
     */
        if(!emptyLocations.isEmpty()){
            int nextLocation = (int)(Math.random() * emptyLocations.size());
            crumbs.push(this.getLocation());
            this.moveTo(emptyLocations.get(nextLocation));
        }else{
         try{
             this.moveTo(this.crumbs.pop());
         } catch(EmptyCollectionException ex){
             System.out.println("There are no Crumbs to back track");
         }
        }
